...
const DEFAULT_BOOKMARKS = [
    ...
    { title: '📚 Читальный зал', links: [{ name: '📕 Author.Today', url: 'https://author.today/' }, { name: '📘 Литрес', url: 'https://www.litres.ru/' }, { name: '📙 Литнет', url: 'https://litnet.com/' }, { name: '🎧️ Книга в ухе', url: 'https://knigavuhe.org/' }, { name: '📑 Самиздат', url: 'http://samlib.ru/' }] },
    ...
    { title: '🎬 Кино & Сериалы', links: [{ name: 'Кинопоиск', url: 'https://hd.kinopoisk.ru/' }, { name: 'IVI', url: 'https://www.ivi.ru/' }, { name: 'Okko', url: 'https://okko.ru/' }, { name: 'Premier', url: 'https://premier.one/' }, { name: 'Wink', url: 'https://wink.ru/' }] },
    ...
    { title: '🏛️ Услуги & Сервисы', links: [{ name: 'Госуслуги', url: 'https://gosuslugi.ru/' }, { name: 'Авиасейлс', url: 'https://www.aviasales.ru/' }, { name: 'Tutu.ru', url: 'https://tutu.ru/' }, { name: 'Сравни.ру', url: 'https://sravni.ru/' }, { name: 'YouDo', url: 'https://youdo.com/' }] },
...
function initLinkSortables() {
    ...
    document.querySelectorAll('#bookmarks-grid .card').forEach(card => {
        ...
        linkSortables.push(Sortable.create(ul, {
            ...
            delay: 250,                    // тач: долгое нажатие — старт перетаскивания
...
// На смартфонах и планшетах фокус при загрузке открывает экранную клавиатуру
// и дёргает страницу снизу вверх — это мешает, а не помогает.
...
// Сбрасывает данные ТОЛЬКО текущей копии: удаляются её ключи (с её префиксом).
// radio_stations намеренно сохраняется: список станций пользователь добавляет вручную.
// Данные соседних папок остаются нетронутыми.
document.getElementById('btn-reset').addEventListener('click', () => {
    if (confirm(
        'Сбросить эту копию дашборда к настройкам по умолчанию?\n\n' +
        'Будут удалены: изменённые закладки и категории, заметки, обои, настройки помодоро.\n' +
        ...
        // Флаг запрещает модулям сохранять данные во время перезагрузки после сброса.
        window.__dashboardResetInProgress = true;

        // Удаляем все ключи текущей копии, кроме radio_stations (см. utils.js).
